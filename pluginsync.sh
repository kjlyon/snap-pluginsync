...
docker run -v "$(pwd)":/plugins -e SSH_AUTH_SOCK=/var/run/ssh_agent.sock  -it intelsdi/pluginsync /bin/bash -c "socat UNIX-LISTEN:/var/run/ssh_agent.sock,reuseaddr,fork TCP:192.168.65.1:${PORT} & cd /plugins && echo 'commands do not be prefixed with bundle exec, try: \"rake -T\" or \"travis\" or \"msync\"' && /bin/bash"
